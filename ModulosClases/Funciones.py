...
def obtener_papelera(usuario_id):
    with conectar() as conexion:
        ...
        cursor.execute(
            """
            SELECT id, tipo, referencia, asunto, contenido, fecha
            FROM (
                SELECT m.id, m.tipo, 'Borrador' AS referencia, m.asunto, m.contenido, m.fecha
                FROM Mensajes m
                WHERE m.tipo = 'borrador' AND m.remitente_id = ? AND m.visible_remitente = 1 AND m.eliminado_remitente = 1
                UNION ALL
                SELECT m.id, m.tipo, COALESCE(c.nombre, c.correo, 'Sin destinatario') AS referencia, m.asunto, m.contenido, m.fecha
                FROM Mensajes m LEFT JOIN Correos c ON c.id = m.destinatario_id
                WHERE m.tipo = 'enviado' AND m.remitente_id = ? AND m.visible_remitente = 1 AND m.eliminado_remitente = 1
                UNION ALL
                SELECT m.id, m.tipo, COALESCE(c.nombre, c.correo, 'Sin remitente') AS referencia, m.asunto, m.contenido, m.fecha
                FROM Mensajes m LEFT JOIN Correos c ON c.id = m.remitente_id
                WHERE m.tipo = 'enviado' AND m.destinatario_id = ? AND m.visible_destinatario = 1 AND m.eliminado_destinatario = 1
            ) ORDER BY fecha DESC
            """,
...
def restaurar_desde_papelera(mensaje_id, usuario_id):
    with conectar() as conexion:
        ...
        cursor.execute(
            ...
            UPDATE Mensajes
            SET
                eliminado_remitente = CASE WHEN remitente_id = ? AND visible_remitente = 1 THEN 0 ELSE eliminado_remitente END,
                eliminado_destinatario = CASE WHEN destinatario_id = ? AND visible_destinatario = 1 THEN 0 ELSE eliminado_destinatario END
            WHERE id = ? AND ((remitente_id = ? AND visible_remitente = 1) OR (destinatario_id = ? AND visible_destinatario = 1))
            """,
            (usuario_id, usuario_id, mensaje_id, usuario_id, usuario_id),
        )
        conexion.commit()
...
def eliminar_definitivamente(mensaje_id, usuario_id=None):
    with conectar() as conexion:
        ...
        if usuario_id is None:
            cursor.execute("DELETE FROM Mensajes WHERE id = ? AND tipo = 'borrador'", (mensaje_id,))
        else:
            cursor.execute(
                """
                UPDATE Mensajes
                SET
                    visible_remitente = CASE WHEN remitente_id = ? AND visible_remitente = 1 THEN 0 ELSE visible_remitente END,
                    visible_destinatario = CASE WHEN destinatario_id = ? AND visible_destinatario = 1 THEN 0 ELSE visible_destinatario END
                WHERE id = ? AND ((remitente_id = ? AND visible_remitente = 1) OR (destinatario_id = ? AND visible_destinatario = 1))
                """,
                (usuario_id, usuario_id, mensaje_id, usuario_id, usuario_id),
            )
